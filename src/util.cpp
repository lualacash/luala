...
const char * const BITCOIN_CONF_FILENAME = "luala.conf";
const char * const BITCOIN_PID_FILENAME = "lualad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "luala" is a composite category enabling all Luala-related debug output
            if(ptrCategory->count(string("luala"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LualaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LualaCore
    // Mac: ~/Library/Application Support/LualaCore
    // Unix: ~/.lualacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LualaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LualaCore";
#else
    // Unix
    return pathRet / ".lualacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty luala.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override luala.conf
